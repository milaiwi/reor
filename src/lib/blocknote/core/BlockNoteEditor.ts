...
export class BlockNoteEditor<BSchema extends BlockSchema = HMBlockSchema> {
  public readonly tiptapEditor: TiptapEditor & { contentComponent: any }
  ...
  public get prosemirrorView() {
    return this.tiptapEditor.view
  }

  public get domElement() {
    return this.tiptapEditor.view.dom as HTMLDivElement
  }

  public isFocused() {
    return this.tiptapEditor.view.hasFocus()
  }

  public focus() {
    this.tiptapEditor.view.focus()
  }
  ...
  public onEditorSelectionChange(callback: () => void) {
    this.tiptapEditor.on('selectionUpdate', callback)
  }
  ...
  public getTextCursorPosition(): TextCursorPosition<BSchema> {
    ...
    const nodeIndex = this.tiptapEditor.state.doc.resolve(endPos).index(depth - 1)
    // Number of the parent blockGroup's child blockContainer nodes.
    const numNodes = this.tiptapEditor.state.doc.resolve(endPos + 1).node().childCount
  ...
  public setTextCursorPosition(targetBlock: BlockIdentifier, placement: 'start' | 'end' = 'start') {
    ...
    const { posBeforeNode } = getNodeById(id, this.tiptapEditor.state.doc)
    const { startPos, contentNode } = getBlockInfoFromPos(this.tiptapEditor.state.doc, posBeforeNode + 2)!

    if (placement === 'start') {
      this.tiptapEditor.commands.setTextSelection(startPos + 1)
    } else {
      this.tiptapEditor.commands.setTextSelection(startPos + contentNode.nodeSize - 1)
    }
  }
  ...
  public getSelection(): Selection<BSchema> | undefined {
    if (this.tiptapEditor.state.selection.from === this.tiptapEditor.state.selection.to) {
      return undefined
    }
    ...
    this.tiptapEditor.state.doc.descendants((node, pos) => {
      ...
      if (pos + node.nodeSize < this.tiptapEditor.state.selection.from || pos > this.tiptapEditor.state.selection.to) {
        return true
      }

      blocks.push(nodeToBlock(this.tiptapEditor.state.doc.resolve(pos).node(), this.schema, this.blockCache))
  ...
  public set isEditable(editable: boolean) {
    this.tiptapEditor.setEditable(editable)
  }
  ...
  public insertBlocks(
    ...
    insertBlocks(blocksToInsert, referenceBlock, placement, this.tiptapEditor)
  }
  ...
  public updateBlock(blockToUpdate: BlockIdentifier, update: PartialBlock<BSchema>) {
    updateBlock(blockToUpdate, update, this.tiptapEditor)
  }
  ...
  public getActiveStyles() {
    const styles: Styles = {}
    const marks = this.tiptapEditor.state.selection.$to.marks()
  ...
  public addStyles(styles: Styles) {
    ...
    for (const [style, value] of Object.entries(styles)) {
      if (toggleStyles.has(style as ToggledStyle)) {
        this.tiptapEditor.commands.setMark(style)
      } else if (colorStyles.has(style as ColorStyle)) {
        this.tiptapEditor.commands.setMark(style, { color: value })
      }
    }
  }
  ...
  public removeStyles(styles: Styles) {
    ...
    for (const style of Object.keys(styles)) {
      this.tiptapEditor.commands.unsetMark(style)
    }
  }
  ...
  public toggleStyles(styles: Styles) {
    ...
    for (const [style, value] of Object.entries(styles)) {
      if (toggleStyles.has(style as ToggledStyle)) {
        this.tiptapEditor.commands.toggleMark(style)
      } else if (colorStyles.has(style as ColorStyle)) {
        this.tiptapEditor.commands.toggleMark(style, { color: value })
      }
    }
  }
  ...
  public getSelectedText() {
    return this.tiptapEditor.state.doc.textBetween(
      this.tiptapEditor.state.selection.from,
      this.tiptapEditor.state.selection.to,
    )
  }
  ...
  public getSelectedLinkUrl() {
    return this.tiptapEditor.getAttributes('link').href as string | undefined
  }
  ...
  public createLink(url: string, text?: string) {
    ...
    if (!text) {
      text = this.tiptapEditor.state.doc.textBetween(from, to)
    }

    const mark = this.tiptapEditor.schema.mark('link', { href: url })

    this.tiptapEditor.view.dispatch(
      this.tiptapEditor.view.state.tr.insertText(text, from, to).addMark(from, from + text.length, mark),
    )
  }
  ...
  public canNestBlock() {
    ...
    return this.tiptapEditor.state.doc.resolve(startPos).index(depth - 1) > 0
  }
   ...
   */
  public nestBlock() {
    this.tiptapEditor.commands.sinkListItem('blockContainer')
  }
  ...
  public canUnnestBlock() {
    const { depth } = getBlockInfoFromPos(this.tiptapEditor.state.doc, this.tiptapEditor.state.selection.from)!
   ...
   */
  public unnestBlock() {
    this.tiptapEditor.commands.liftListItem('blockContainer')
  }
  ...
  public async HTMLToBlocks(html: string): Promise<Block<BSchema>[]> {
    return HTMLToBlocks(html, this.schema, this.tiptapEditor.schema)
  }
  ...
  public async markdownToBlocks(markdown: string): Promise<Block<BSchema>[]> {
    return markdownToBlocks(markdown, this.schema, this.tiptapEditor.schema)
  }
  ...
  public updateCollaborationUserInfo(user: { name: string; color: string }) {
    ...
    this.tiptapEditor.commands.updateUser(user)
  }
}
