...
import { Files, MessageCircle, Search, Moon, SunMoon } from 'lucide-react'
...
const IconsSidebar: React.FC<IconsSidebarProps> = ({ getShortcutDescription }) => {
  ...
  return (
    <div className="flex size-full w-[55px] flex-col items-center justify-between gap-1 bg-neutral-800 pt-2">
      ...
      >
        <div
          className={`flex size-4/5 items-center justify-center rounded hover:bg-gray-700 ${
            sidebarShowing === 'files' ? 'bg-gray-600' : ''
          }`}
        >
          <div title={getShortcutDescription('open-files') || 'Files'}>
            <Files size={20} color={determineColor('files')} />
          </div>
        </div>
      </div>
      <div
      ...
      >
        <div
          className={`flex size-4/5 items-center justify-center rounded hover:bg-gray-700 ${
            sidebarShowing === 'chats' ? 'bg-gray-600' : ''
          }`}
        >
          <div title={getShortcutDescription('open-chat-bot') || 'Open Chatbot'}>
            <MessageCircle
              size={20}
      ...
      >
        <div
          className={`flex size-4/5 items-center justify-center rounded hover:bg-gray-700 ${
            sidebarShowing === 'search' ? 'bg-gray-600' : ''
          }`}
        >
          <div title={getShortcutDescription('open-search') || 'Semantic Search'}>
            <Search
              size={20}
      ...
      >
        <div className="flex size-4/5 items-center justify-center rounded hover:bg-gray-700">
          <HiOutlinePencilAlt
      ...
      <div className="flex h-8 w-full cursor-pointer items-center justify-center border-none bg-transparent">
        ...
        >
          {state === 'dark' ? (
            <div title={getShortcutDescription('toggle-theme') || 'Toggle Theme'}>
              <SunMoon size={20} color="gray" />
            </div>
          ) : (
            <div title={getShortcutDescription('toggle-theme') || 'Toggle Theme'}>
